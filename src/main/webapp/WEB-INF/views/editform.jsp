...
<div style="width: 700px; margin: auto; ">
	...
	<div class="form-row">
		...
		<div class="form-group col-md-4">
			<label>시간</label>
			<input type="text" class="form-control" value="<fmt:formatDate value="${vo.noticeDate }" pattern="yyyy-MM-dd"/>">
		</div>
	</div>
  ...
  <c:if test="${role eq '[ROLE_ADMIN]' }">
  	<button type="submit" class="btn btn-danger" formaction="javascript:noticeDelete('${vo.idx }')">삭제</button>
	<button type="submit" class="btn btn-primary">수정</button>
	<button type="submit" class="btn btn-primary" formaction="notice">취소</button>
  </c:if>
  <c:if test="${role eq '[ROLE_USER]' }">
	<button type="submit" class="btn btn-primary" formaction="notice">목록</button>
  </c:if>
  </div>
